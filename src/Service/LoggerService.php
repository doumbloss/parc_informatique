...
class LoggerService
{
    ...
    public function logDeletion(object $entity, array $additionalContext = []): void
    {
        ...
        $context = array_merge([
            'deleted_entity_data' => $this->extractEntityData($entity),
            'timestamp' => (new \DateTime())->format('Y-m-d H:i:s')
        ], $additionalContext);
    ...
    private function extractEntityData(object $entity): array
    {
        if ($entity instanceof Panne) {
            return [
                ...
                'equipement_id' => $entity->getEquipement() ? $entity->getEquipement()->getId() : null,
            ];
        } elseif ($entity instanceof Equipement) {
            return [
                'id' => $entity->getId(),
                'codeInventaire' => $entity->getCodeInventaire(),
                'nom' => $entity->getNom(),
